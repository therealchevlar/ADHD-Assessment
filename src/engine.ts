...
export function calculate(a: Assessment) {
  ...
  const imp = impairmentValues.length ? impairmentValues.reduce<number>((n, v) => n + v, 0) / impairmentValues.length : 0;
  const diff = Object.values(a.differential).filter(v => v >= 2).length;
  const executiveValues = Object.values(a.executive);
  const executiveAverage = executiveValues.length ? executiveValues.reduce<number>((n, v) => n + v, 0) / executiveValues.length : 0;
